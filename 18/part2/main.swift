...
do {
	...
	fileURL.processLineByLine { (line: String) in
		let pairString = line
		allPairs.append(pairString)
	}

	var highestMagnitude = 0
	for firstString in allPairs {
		for secondString in allPairs {
			guard (firstString != secondString) else {
				continue
			}

			let first = PairBuilder.build(from: firstString)
			let second = PairBuilder.build(from: secondString)
			let combined = first.add(second)
			let firstMagnitude = magnitude(of: combined)
			if (firstMagnitude > highestMagnitude) {
				highestMagnitude = firstMagnitude
			}
		}

		print("Checked \(firstString)")
	}
...
func magnitude(of pair: Pair) -> Int {
	var continueResolve = true
	while continueResolve {
		let explosions = pair.resolveExplosion()
		if (explosions == false) {
			continueResolve = pair.resolveSplit()
		}
	}
