...
do {
	...
	for foldAction in folds {
		paper = fold(foldAction, onPaper: paper)
	}

	// y: string
	var drawing = [Int: String]()
	for point in paper {
		let string = drawing[point.y] ?? String(repeating: " ", count: 50)
		let index = string.index(string.startIndex, offsetBy: point.x)
		let range = index..<string.index(after: index)
		drawing[point.y] = string.replacingCharacters(in: range, with: "-")
	}
