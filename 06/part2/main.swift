...
do {
	...
	fileURL.processLineByLine { (line: String) in
		...
		for fish in fishes {
			let value = days[fish.daysBeforeProduce] ?? 0
			days[fish.daysBeforeProduce] = value + 1
		}

		for i in 0..<256 {
			var newBatch = [Int: UInt64]()
			for (day, amount) in days {
				if (day - 1 < 0) {
					newBatch[8] = amount

					newBatch[6] = (newBatch[6] ?? 0) + amount
				} else {
					newBatch[day - 1] = amount + (newBatch[day - 1] ?? 0)
				}
			}
