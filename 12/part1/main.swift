...
do {
	...
	let allPaths = findAllPathsFromStart(map: map)
...
func findAllPathsFromStart(map: [Cave: [Cave]]) -> [[Cave: [Int]]] {
	let start = Cave.start
	let way = [start: [0]]
	return findAllPaths(from: start, jump: 1, way: way, map: map)
}

func findAllPaths(from cave: Cave, jump: Int, way: [Cave: [Int]], map: [Cave: [Cave]]) -> [[Cave: [Int]]] {
	var allPaths = [[Cave: [Int]]]()
	let paths = map[cave]
	for next in paths! {
		var currentPath = way
		guard next.isBig == true || way[next] == nil else {
			continue
		}
		...
		if (next != Cave.end) && (cameBackTooMuch == false) {
			allPaths.append(contentsOf: findAllPaths(from: next, jump: jump + 1, way: currentPath, map: map))
