...
import Foundation
	...
	func getValue(fromDiff: Int) -> Int {
		if (fromDiff <= 0) {
			return fromDiff
		}
		let realValue = (1...fromDiff).reduce(0) { (result: Int, value: Int) in
			return result + value
		}
	...
	func diff(average: Int, dict: [Int: Int], before: Bool, max: Int) -> (totalDiff: Int, closest: Int) {
		...
		for position in (average...max) {
			...
			let temp =  abs(average - position)

			totalDiffAfter += getValue(fromDiff: temp) * value
		...
		for position in 0..<average {
			...
			totalDiffBefore += getValue(fromDiff: temp) * value
